...
def train(model, train_loader, test_loader):
    ...
    for epoch in range(model.opt.starting_epoch_count+1, model.opt.n_epochs+1): # opt.niter + opt.niter_decay + 1):
        ...
        if (epoch == model.opt.starting_epoch_count + 1):
            ...
            print('Saving initial best model at epoch %d (median=%.4f)' % (epoch, best_median))
            model.save_networks('best')
        else:
            new_eval_loss, new_med_error = test(model, test_loader, save_output=False)
            if new_med_error < best_median:
                best_median = new_med_error
                print('Saving new best model at epoch %d (median=%.4f)' % (epoch, best_median))
                model.save_networks('best')
...
def test(model, test_loader, save_output=True, save_name="decoder_test_result", save_dir="", log=True):
    ...
    if log:
        ...
        if opt_exp.n_decoders == 2:
            write_log([str(total_offset_loss)], model.offset_decoder.model_name, log_dir=model.opt.log_dir, log_type='test_offset_loss')
