...
The DLoc baseline uses its original configuration: SGD with StepLR scheduling ($\gamma = 0.9$), with step sizes of 50 (encoder), 20 (decoder), and 50 (offset decoder) epochs.
...
All models are trained exclusively on Sessions~1\,\&\,2 and evaluated on each session without any fine-tuning.

\begin{table}[H]
\centering
\caption{Cross-session generalization --- median localization error (meters). \best{Green bold} = best in column.}
...
\begin{table}[H]
\centering
\caption{Cross-session generalization --- P90 localization error (meters).}
...
\begin{table}[H]
\centering
\caption{Cross-session degradation relative to in-distribution baseline (Session~2) --- median error.}
...
\subsection{Comparison with DLoc Paper}

The original DLoc paper~\cite{ayyalasomayajula2020deep} reports cross-session results using a \textbf{leave-one-session-out} protocol, where the model trains on 3 sessions (including August sessions) and tests on 1 held-out session.
Our setup is more challenging: we train on \textbf{only 2 July sessions} (Sessions~1\,\&\,2) and test on August sessions that the model has never seen.

\begin{table}[H]
\centering
\caption{Comparison with DLoc paper's cross-session results (median error in meters). Note different training protocols: paper trains on 3 sessions (including Aug16), we train on 2 July sessions only.}
\label{tab:paper_comparison}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Test Session} & \textbf{Paper DLoc} & \textbf{Our MobileNet} & \textbf{Our U-Net} & \textbf{Our U-Net (D)} \\
 & \footnotesize{(3-sess train)} & \footnotesize{(2-sess train)} & \footnotesize{(2-sess train)} & \footnotesize{(2-sess train)} \\
\midrule
Session 3 (furniture)  & 0.71\,m & 1.118\,m & 1.063\,m & \best{1.020\,m} \\
Session 4 (diff.\ furn.) & 0.82\,m & 1.204\,m & 1.005\,m & \best{1.005\,m} \\
Session 5 (reflector)  & 1.05\,m & 1.500\,m & 1.334\,m & \best{1.281\,m} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Our U-Net approaches paper DLoc despite training on fewer sessions.}
On Session~4, our U-Net (1.005\,m with only 2-session training) approaches the paper's DLoc result (0.82\,m with 3-session training).
On Session~5, our U-Net (1.281\,m) approaches the paper's 1.05\,m.
Given that our experimental setup is strictly harder (50\% less training data, no August sessions in training), these results suggest that the U-Net architecture extracts more generalizable features from limited training data than the original DLoc encoder.
...
\begin{itemize}[nosep]
  \item \textbf{Optimizer:} SGD with learning rate $10^{-5}$
...
\paragraph{Finding 6: Cross-session generalization is achievable with limited training data.}
The distilled U-Net, trained on only 2 July sessions, achieves 1.02--1.28\,m median error on unseen August sessions with furniture changes and metal reflectors.
For comparison, the original DLoc paper reports 0.71--1.05\,m using a leave-one-session-out protocol (3-session training including August data).
Our model approaches these results with 50\% less training data and a strictly harder evaluation protocol, suggesting that the combination of U-Net skip connections and knowledge distillation extracts more generalizable features from limited observations.
...
\begin{itemize}
  ...
  \item \textbf{Real-time inference:} We report model size but not inference latency on target edge devices. FLOPs analysis and on-device benchmarking would complete the deployment picture.
...
%% ####################################################################
\addcontentsline{toc}{section}{Question for Discussion}

\vspace{0.5em}
\noindent\fbox{\parbox{0.97\linewidth}{
\textbf{Regarding Step~4 --- Multi-Seed Statistical Validation:}\\[0.5em]
Given the limited computational budget remaining, should we prioritize running multi-seed experiments on the \textbf{student models} (MobileNetV2, TinyCNN, U-Net in standalone and distilled modes) or on the \textbf{DLoc baseline} (which has already been completed for 3 seeds)?
\\[0.5em]
The student multi-seed runs require approximately 15 hours on an H100 GPU for 6 variants $\times$ 3 seeds $\times$ 120 epochs.
All scripts, weights, and data are uploaded to HuggingFace and ready for immediate execution.
}}
